...
use nusb::transfer::{ControlOut, ControlType, Direction, EndpointType, Recipient, RequestBuffer};
use nusb::Interface;
use std::ffi::c_void;
...
unsafe extern "C" fn transport_can_send_packet_now(packet_type: u8) -> c_int {
    if packet_type != HCI_COMMAND_DATA_PACKET as u8 && packet_type != HCI_ACL_DATA_PACKET as u8 {
        return 0;
    }
...
unsafe extern "C" fn transport_set_sco_config(_voice_setting: u16, _num_connections: c_int) {}
...
fn is_candidate_device(device: &nusb::DeviceInfo, selection: UsbSelection) -> bool {
    ...
    selection.vendor_id.is_some() || selection.product_id.is_some() || is_bluetooth_device(device)
}
...
fn detect_endpoints(interface: &Interface) -> EndpointConfig {
    ...
    thread::spawn(move || {
        while !stop.load(Ordering::Relaxed) {
            ...
            let result = match packet {
                OutgoingPacket::Command(packet) => send_hci_command(&interface, &packet),
                OutgoingPacket::Acl(packet) => send_acl_data(&interface, acl_out_endpoint, &packet),
            };
            pending.fetch_sub(1, Ordering::Relaxed);
...
fn send_acl_data(interface: &Interface, acl_out_endpoint: u8, packet: &[u8]) -> Result<(), ()> {
    let transfer = interface.bulk_out(acl_out_endpoint, packet.to_vec());
    block_on(transfer).into_result().map(|_| ()).map_err(|_| ())
}
